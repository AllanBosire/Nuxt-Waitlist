...
export type CustomBotClient = Omit<
    InstanceType<typeof clients.Client4>,
    "userId"
> & {
...
export async function getOrCreateDM(details: {
    bot: Bot;
    message?: string;
    user_id: string;
}) {
    ...
    if (!dmChannel) {
        const { result, error: dmError } = await execute(
            bot.createDirectChannel,
            [await bot.userId(), details.user_id],
        );
        if (dmError && !result) {
            throw createError({
        ...
        getWebSocket() {
            ...
            const connect = (): WebSocket => {
                ...
                ws.on("message", (raw) => {
                    ...
                    if (
                        message.hasOwnProperty("seq_reply") ||
 ...
 */
export async function showTyping<T>(
    bot: Bot,
    channelId: string,
    promise: Promise<T>,
) {
